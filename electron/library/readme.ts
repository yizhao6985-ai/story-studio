...
export function parseReadmeContent(raw: string): Omit<ReadmeInfo, "raw"> {
...
export function buildReadmeTemplate(title: string, author?: string): string {
  const lines = [`# ${title.trim()}`, ""];
  if (author?.trim()) {
    lines.push(`**作者** ${author.trim()}`, "");
  }
  lines.push("## 简介", "", "");
  return lines.join("\n");
}
