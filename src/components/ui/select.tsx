...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-7 items-center justify-between gap-2 rounded-none border border-border bg-input px-2 text-xs text-foreground focus:outline-none focus:ring-1 focus:ring-ring disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1",
      className,
    )}
    {...props}
  >
  ...
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ...
      className={cn(
        ...
        !footer && "overflow-y-auto overflow-x-hidden",
        className,
      )}
      position={position}
      {...props}
    >
      ...
      <SelectPrimitive.Viewport
        className={cn(
          footer ? "max-h-56 overflow-y-auto p-1" : "p-1",
          position === "popper" &&
            !footer &&
      ...
      {footer ? (
        <div className="shrink-0 border-t border-border p-1">{footer}</div>
      ) : null}
